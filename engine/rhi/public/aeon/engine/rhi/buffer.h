...
protected:
    explicit buffer(const std::uint32_t size, const engine::rhi::usage usage) noexcept
        : size_{size}
        , usage_{usage}
    {
    }
...
protected:
    explicit index_buffer(const std::uint32_t size, const engine::rhi::usage usage) noexcept
        : buffer{size, usage}
    {
    }
};

using index_buffer_ref = aeon::common::intrusive_ptr<index_buffer>;
...
protected:
    explicit vertex_buffer(const std::uint32_t size, const engine::rhi::usage usage) noexcept
        : buffer{size, usage}
    {
    }
};

using vertex_buffer_ref = aeon::common::intrusive_ptr<vertex_buffer>;
...
protected:
    explicit constant_buffer(const std::uint32_t size, const engine::rhi::usage usage) noexcept
        : buffer{size, usage}
    {
    }
};

using constant_buffer_ref = aeon::common::intrusive_ptr<constant_buffer>;
