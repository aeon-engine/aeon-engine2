...
// void import_materials(const aiScene *scene)
//{
...
auto assimp_codec::import_multi(const core::types::path &path) -> import_result
{
...
auto assimp_codec::format_support_type(const std::u8string &extension) const noexcept -> format_type
{
    if (importer_.IsExtensionSupported(reinterpret_cast<const char *>(extension.c_str())))
